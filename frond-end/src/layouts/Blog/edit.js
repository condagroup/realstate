...
import {setReadingTime} from 'layouts/Blog/functions';
import CKEditor from 'components/CKEditor/index';
import { categories } from './functions';


function AddBlogForm({close,successAlert,loadBlogs,blog}) {
  const [picture, setPicture] = useState('');
  const [title, setTitle] = useState(blog.title);
  const [description, setDescription] = useState(blog.description);
  const [section, setSection] = useState(blog.section);
  const [readTime, setReadTime] = useState(blog.readTime);
  const [file,set_file]=useState(null);
  const [sending,set_sending]=useState(false);
  const [categorie,set_categorie]=useState(blog.category)
  const [tags,set_tags]=useState(blog.tags)
  ...
  const  handleSubmit=async (event) =>{
    ...
    fetch(apiConstants.UPDATE_BLOG, requestOptions).then(async (response) => {
    ...
    <form onSubmit={handleSubmit} className="text-sm">
       <div className="flex flex-col mb-4">
        ...
        >
          ...
          {categories?.map((item)=>{
            let selected=false;
            console.log("the cat",categorie)
            if(item?.id==categorie){
                selected=true;
            }
            return (
              <option key={item?.id} selected={selected} value={item?.id}>{item?.title}</option>
            )
          })}
        </select>
      </div>
      ...
      <div className="flex flex-col mb-4">
        ...
        <a className="text-blue-500" target="_blank" href={apiConstants.BASE_URL+"/uploads/"+blog.picture}>
           {blog.picture}
        </a>
      </div>
